            ...
            >
              ...
              <div className="absolute inset-0 w-full h-full">
                <Image
                  ...
                  alt={item.title}
                  fill
                  className="object-cover opacity-20 filter blur-2xl scale-110 transform origin-center"
                  priority={index === 0}
                  unoptimized
                />
                <div className="absolute inset-0 bg-gradient-to-t from-bg-primary via-bg-primary/95 to-transparent" />
                <div className="absolute inset-0 bg-gradient-to-r from-bg-primary/90 via-transparent to-transparent md:from-bg-primary md:via-bg-primary/50" />
              </div>

              {/* Content Container */}
              <div className="relative w-full h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center pt-24 pb-14 md:pt-20 md:pb-10">
                <div className="flex flex-col md:flex-row items-center justify-center md:justify-between w-full gap-6 md:gap-8 lg:gap-16">
                  
                  {/* Left: Text & Actions */}
                  <div className={clsx("w-full md:w-3/5 xl:w-1/2 flex flex-col items-center md:items-start text-center md:text-left order-2 md:order-1 transition-all duration-700 delay-100", isActive ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0")}>
                    ...
                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-3 md:mb-4">
                      ...
                      {item.score && (
                        <div className="flex items-center gap-1 px-2.5 py-0.5 md:px-3 md:py-1 text-[10px] md:text-xs font-bold text-accent-yellow bg-accent-yellow/10 border border-accent-yellow/20 rounded-full backdrop-blur-sm">
                          <Star className="w-3 md:w-3.5 h-3 md:h-3.5 fill-accent-yellow" />
                    ...
                    {/* Title */}
                    <h1 className="text-2xl sm:text-3xl md:text-5xl lg:text-7xl font-black text-white tracking-tight leading-[1.2] md:leading-[1.1] mb-3 md:mb-6 drop-shadow-2xl line-clamp-2 md:line-clamp-3">
                      {item.title}
                    </h1>

                    {/* Synopsis */}
                    <p className="text-xs sm:text-sm md:text-lg text-text-secondary font-medium leading-relaxed max-w-2xl line-clamp-2 md:line-clamp-3 mb-6 md:mb-8 drop-shadow-md">
                  ...
                  {/* Right: Poster Image */}
                  <div className={clsx("flex w-[140px] sm:w-[180px] md:w-2/5 xl:w-1/3 items-center justify-center order-1 md:order-2 transition-all duration-1000 delay-300", isActive ? "translate-y-0 md:translate-x-0 opacity-100 scale-100" : "translate-y-4 md:translate-x-12 opacity-0 scale-95")}>
                    <div className="relative w-full aspect-[2/3] rounded-2xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.6)] border border-white/10 group">
                      <Image
                        src={item.thumbnail}
                        alt={item.title}
                        fill
                        ...
                        unoptimized
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-bg-primary/40 via-transparent to-transparent opacity-80" />
                    </div>
                  </div>
                </div>
              </div>
